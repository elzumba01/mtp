...
export function cryptoHealth() {
  ...
  return {
    configured: !!merchant && ethers.isAddress(merchant),
 ...
 * en stablecoin. Usamos paridad simple 1 EUR = 1 USDC (en producción
 * conviene una cotización real EUR/USD vía un oracle como Chainlink).
 *
 * Ejemplo: 2900 céntimos (29 €) → 29 USDC → BigInt con los decimales correctos.
 */
function eurToStableUnits(amountCents, decimals) {
  // 1 EUR ≈ 1 USDC. Pasamos centavos → string con punto decimal → BigInt
  const eurStr = (amountCents / 100).toFixed(2);
  return ethers.parseUnits(eurStr, decimals);
}
 ...
 */
export function getCryptoQuote({ amountCents, chainId, token }) {
  const merchant = process.env.CRYPTO_MERCHANT_ADDRESS || '';
  ...
  const amountUnits = eurToStableUnits(amountCents, tokenInfo.decimals);
